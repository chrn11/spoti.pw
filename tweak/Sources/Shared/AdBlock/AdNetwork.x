...
%ctor {
    ...
    // Spotify 9.1.84+ moved the product state off the legacy SPTDataLoaderService; the modern
    // transport is Connectivity's HttpClientURLSession, which is hooked below as well. The rewrite
    // stays on while either transport is present, and says so when neither is.
    if (premium && !NSClassFromString(@"SPTDataLoaderService") && !NSClassFromString(@"_TtC26Connectivity_HttpClientKit20HttpClientURLSession")) {
        premium = NO;
        SGLog(@"adblock: no known Spotify transport for the premium network rewrite, leaving it off");
    }
